...
function createToolInstance({
  ...
  const _call = async (toolArguments, config) => {
    ...
    try {
      ...
      /**
       * Fix for MCP tool response handling:
       * formatToolContent returns [content, artifact] where content can be:
       * - A string (for unrecognized providers)
       * - A FormattedContent[] array (for recognized providers like openai)
       *
       * When content is an array, it may be empty [] if the MCP tool returned
       * data that didn't produce text blocks. In that case, we need to ensure
       * the content is a non-empty string so the LLM can see the tool response.
       *
       * Additionally, for providers that return content as an array of
       * {type: 'text', text: '...'} objects, we flatten to a plain string
       * to ensure maximum compatibility with all LLM providers (especially
       * custom endpoints routed through proxies like LiteLLM).
       */
      if (Array.isArray(result) && result.length === 2) {
        let [content, artifact] = result;

        // If content is an array, convert to string for reliable transport
        if (Array.isArray(content)) {
          if (content.length === 0) {
            // Empty content array - try to extract raw text from MCP response
            content = '(No response)';
          } else {
            // Flatten content array to plain text string
