            ...
            const results: ToolExecuteResult[] = await Promise.all(
              toolCalls.map(async (tc: ToolCallRequest) => {
                ...
                try {
                  ...
                  /**
                   * Ensure tool result content is always a non-empty string.
                   * MCP tools may return content as an array of {type, text} objects
                   * which needs to be flattened to a string for reliable transport
                   * through LLM providers (especially custom endpoints via LiteLLM).
                   */
                  let resultContent = result.content;
                  if (Array.isArray(resultContent)) {
                    const textParts = (resultContent as Array<Record<string, unknown> | string>)
                      .map((item) => {
